...
struct common_params_speculative_draft {
    ...
    // physical ubatch of the draft context. 0 means "inherit the main model's",
    // which was the only behaviour before this addition. The draft crosses the
    // whole prompt as well, in chunks of n_ubatch, so its compute buffer grows
    // with the target's ubatch without needing to.
    int32_t n_ubatch = 0;
...
struct common_params {
    ...
    int32_t n_kv_reserve          =     0; // n_kv used to size the compute buffers, 0 == full context
